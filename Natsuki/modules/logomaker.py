...
async def lego(event):
 ...
 if event.sender_id == OWNER_ID:
    ...
    if not quew:
       ...
       pass
 await event.reply('✍️Designing your logo...wait🌺!')
 try:
    ...
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Design by @TheAlexiabot 🌺")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Oops! some error occured go to @AlexiabotDiscussion 🌺, {e}')
...
async def lego(event):
 ...
 if event.sender_id == OWNER_ID:
    ...
    if not quew:
       ...
       pass
 await event.reply('✍️Designing your logo.. using AI power🔥!')
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./AnkiVector/resources/blackbg.jpg')
    draw = ImageDraw.Draw(img)
...
async def lego(event):
 ...
 try:
    ...
    fname2 = "AnkiVectorLogo.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Design by @Lucy_AI_assistant_bot 🔥")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Oops! some error occured go to @AlexiabotDiscussion 🔥, {e}')
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./AnkiVector/resources/Logo-Spiderman.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 25
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./AnkiVector/resources/SpiderWritten.ttf", 275)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./AnkiVector/resources/LucyAi.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "red"
    shadowcolor = "pink"
    font = ImageFont.truetype("./AnkiVector/resources/Deadly Advance 08888.ttf", 70)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.11)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/3), text, font=font, fill=(255, 255, 255))
    x = (image_widthz-w)/2
    y= ((image_heightz-h)/2-6)
    draw.text((x, y), text, font=font, fill="white", stroke_width=5, stroke_fill="red")
    fname2 = "LucyAiLogo.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Design by @TheAlexiabot 🌺")
    if os.path.exists(fname2):
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./AnkiVector/resources/Windowsbg.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./AnkiVector/resources/Graphic_mobile7.ttf",250)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
...
__help__ = """
 ...
 🔥 /pandalogo text :
 🔥 /spiderlogo text :
 🔥 /spidermanlogo text : create Spiderman logo
 🔥/windowslogo text : Create a logo with windows background
 🔥 /lucylogo text :Lucy's profile logo
 """
